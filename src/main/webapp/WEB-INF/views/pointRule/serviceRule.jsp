...
<script type="text/javascript" src="${ctx}/js/pointRule/serviceRule.js"></script>
...
<body>
	...
	<div class="am-g" style='margin: 1px; padding: 10px'>
		<a class="am-btn am-btn-success am-btn-xs" style="border-radius: 300px;" href="${ctx}/serviceRule/index.do">服务刻数产生分数规则</a>
		&emsp;&emsp;
		<a class="am-btn am-btn-default am-btn-xs" style="border-radius: 300px;" href="${ctx}/starsRule/index.do">获得评分产生分数规则</a>
		&emsp;&emsp;
		<a class="am-btn am-btn-default am-btn-xs" style="border-radius: 300px;" href="${ctx}/fiveCountRule/index.do">好评额外奖励分数规则</a>
	</div>
	<hr>
	...
	<div id="content" style='margin: 10px; padding: 10px'>
		<!-- 操作栏 -->
		<div class="am-g">
			<a href="${ctx}/serviceRule/index.do?cmd=save"
				class="am-btn am-btn-success am-btn-xs"
				style="border-radius: 300px;" role="button">添加</a>
		</div>

		<!-- 查询栏 -->
		<form id="serviceRule-search-form" method="post">
		...
		<!-- 数据表格 -->
		<table id="serviceRule-datagrid"></table>
	</div>
	...
	<script type="text/javascript">
		...
		var search = function(){
			// 获取搜索表单 
			// 封装表单参数
			var paramObj = $('#serviceRule-search-form').serializeJson();
			// 发送post请求刷新 datagrid 的数据
			$('#serviceRule-datagrid').datagrid("load",paramObj);
		};
		// 搜索确定按钮
