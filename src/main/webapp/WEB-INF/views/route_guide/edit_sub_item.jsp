...
<div class="am-u-lg-10 am-u-md-12 am-margin-top">
		<h3 style="text-align: center;">
			${item == null || item.id == null ? "添加" : "修改"}子标题
		</h3>
		<form id="form-item" class="am-form am-form-horizontal" enctype="multipart/form-data" method="post">
			<input type="hidden" name="id" id="route-id" value="${item.id}">
			<input type="hidden" name="superId" id="route-id" value="${superId == null ? item.superId : superId}">
			<div class="am-form-group">
				<label for="doc-ipt-3-a" class="am-u-sm-2 am-form-label">标题名称：</label>
				<div class="am-u-sm-10">
					<input name="name" type="text"  id="doc-ipt-3-a" class="am-form-field" placeholder="请输入路线名称" value="${item.name}">
				</div>
			</div>
			<div class="am-form-group">
			...
			<div class="am-form-group">
				<div class="am-u-sm-10 am-u-sm-offset-2">
					...
					<input type="button" class="am-btn am-btn-success am-btn-sm" onclick="location.href='${ctx}/routeguide/index.do?routeId=${routeId}'"  value="返回"></input> 
				</div>
			</div>
		</form>
	</div>
   ...
   <script type="text/javascript">
	   ...
	   $("#confirm").click(function(){
		   $("#form-item").ajaxSubmit({
		       url:       _ctx + "/routeguide/editSubItem.do", 
		       type:      "post",
		       data: {
		       },        
		       clearForm: false,     
		       ...
		       success: function(data){
		    	   if(isJsonObj(data)){
			    	   ...
			    	   modal.find(".data-am-modal-confirm").on("click", function(){
				    	   ...
				    	   if(data.success) {
				    		   location.href = _ctx + "/routeguide/index.do?routeId=${routeId}";
				    	   }
			    	   });
			    	   modal.modal();
		    	  }else {
