...
$(function(){
	// 缓存界面组件
	var serviceStationEditForm = $("#form-service-station-edit");
	...
	var submitEditForm = function (successDel){
    	var data = {};
    	$(".service-types:checked").each(
    			function(i){
    				data['serviceTypes['+ i+'].type'] = this.value;
    				var price=$(this).attr("price");
					if(price) {
						data['serviceTypes['+ i+'].price'] = price;
					}
    			}
    	);
    	var districtAddress = '';
    	...
    	serviceStationEditForm.ajaxSubmit( {    
    	       url:       _ctx + "/servicestation/edit.do", 
    	       type:      "post",
    	       data: data,        
    	       clearForm: false,     
    	       resetForm: false,     
		...
		'confirm':function(){
			submitEditForm(function(data){
			   var title = "提示";
  	    	   ...
  	    	   if(isJsonObj(data)){
  	    		   if(data.success) {
  	    			   location.href = _ctx + "/servicestation/index.do";
  	    			   return;
  	    		   }
  	    		   content = data.msg;
  	    	   }
