...
$(function() {
	...
	//2、缓存组件
	cooperatorGrid = $("#cooperator-datagrid");
	cooperatorSearchForm = $("#cooperator-search-form");
	//3、初始化组件
	// 初始化 数据表格
	cooperatorGrid.datagrid(
		{
			//请求地址
		    url: _ctx + "/cooperatorHotel/list.do",
		    //大小自适应
		    fit:false,
		    //无边框
		    border:false,
		    ...
		    columns:[[    
		        ...
		        {field:'stars',title:'酒店星级',width:5,align:'center'},
		        ...
		        {field:'minimun',title:'最低商品价格(元)',width:10,align:'center'},
		        {field:'outsideImgs',title:'酒店图片',width:0,align:'center',formatter:function(v,r,i){
//		        	return r.outsideImgs;
		        	var picArry=JSON.stringify(r.outsideImgs);
		        ...
		        {field:'status',title:'操作',width:10,align:'center',formatter:function(v,r,i){
		        	if(v == -1){
		        		return "<a href='"+_ctx+"/cooperatorHotel/index.do?cmd=update&id="+r.id+"'>编辑</a>--<a href='javascript:void(0);' onclick='changeStatus("+r.id+",0)' style='color:green;'>开通</a>";
		        	}else if(v == 0){
		        		return "<a href='"+_ctx+"/cooperatorHotel/index.do?cmd=update&id="+r.id+"'>编辑</a>--<a href='javascript:void(0);' onclick='changeStatus("+r.id+",-1)' style='color:red;'>关闭</a>";
		        	}else{
		        		return "<a href='"+_ctx+"/cooperatorHotel/index.do?cmd=update&id="+r.id+"'>编辑</a>";
		        	}
		        }}
		        ]],
